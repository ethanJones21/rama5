...
import { getInvitedMessages } from '../../store/app/messageSlice';
...
const InvitedPage = () => {
  ...
  const { messagesInvited, pageInvited, hasMoreInvited, loading } = useSelector(
    (state) => state.message
  );
  ...
  const handleGetMoreMessages = () => {
    if (hasMoreInvited && !loading) {
      dispatch(getInvitedMessages({ page: pageInvited }));
    }
  };
  return (
    <>
      <div className="w-full h-screen relative" style={{ background: '#F4F5FA' }}>
        <Header
          BackGround="#051B34"
          Title="Invitados"
          Data={profileGroup?.groups_rooms || []}
          Icon={IconMenu}
          Height="60px"
        ...
        >
          <div className="w-full">
            <ChatMessage
              size={messagesInvited?.length}
              DataChat={messagesInvited}
              hasMore={hasMoreInvited}
              ShowIcons
              isActive
              redirectPrivate
              _onScroll={handleGetMoreMessages}
            />
          </div>
        </div>
