...
const InterestPage = () => {
  ...
  return (
    ...
    >
      ...
      <AlertCustom />
      <div
        className="grid gap-4 grid-cols-2 auto-rows-auto py-8 z-0 mx-auto"
      ...
      >
        ...
        {dataInterest?.map((item) => (
          // eslint-disable-next-line jsx-a11y/click-events-have-key-events,jsx-a11y/no-static-element-interactions
          <div
            className={` text-center shadow-lg rounded-4 mx-4 my-4 ${
              item?.is_add ? ' ' : 'border-1 border-gray-200'
            }`}
            style={{
              height: 'max-content',
              backgroundColor: `${item.is_add ? '#051B34' : '#FFF'}`,
            }}
            onClick={() => {
          ...
          >
            <span
              className="text-8 sm:text-10 font-600 px-4 py-2"
              style={{ color: `${item.is_add ? '#FFF' : '#051B34'}` }}
            >
              {item.name}
            </span>
          </div>
