...
const groupUserRepository = {
  rejectGroupUser: async (groupUser) => {
    // eslint-disable-next-line no-useless-catch
    try {
      return await http.put(`${groupUserRoomUrl}/rejected/${groupUser}`,serialize({ _method: 'PUT' }));
    } catch (error) {
      throw error;
    }
  },
  acceptGroupUser: async (groupUser) => {
    // eslint-disable-next-line no-useless-catch
    try {
      return await http.put(
        `${groupUserRoomUrl}/accept/${groupUser}`,
        serialize({ _method: 'PUT' })
      );
    } catch (error) {
      throw error;
    }
